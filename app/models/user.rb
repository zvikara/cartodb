...
FUNCTION
  ...
  def load_cartodb_functions(extensions_only=false, files=[])
    ...
    #in_database(as: :superuser).run("ALTER EXTENSION cartodb UPDATE TO '0.2.0devnext'; ALTER EXTENSION cartodb UPDATE TO '0.2.0dev';")

    in_database(as: :superuser).run('SELECT cartodb.cdb_enable_ddl_hooks();')

    # TODO: This might fail provisioning new users. Needs testing
