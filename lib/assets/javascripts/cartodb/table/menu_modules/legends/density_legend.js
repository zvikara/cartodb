...
cdb.admin.mod.DensityLegend = cdb.admin.mod.CustomLegend.extend({
  ...
  _setupSync: function() {
    ...
    if (this.items && this.items.length > 1 && this.items.at(0).get("legend_type") == "choropleth") {
  ...
  _calculateItems: function() {
    ...
    items.push(new cdb.geo.ui.LegendItemModel({ legend_type: "choropleth", type: "text", name: "Left label",  sync: this.leftSync, value: this.leftLabel }));
    items.push(new cdb.geo.ui.LegendItemModel({ legend_type: "choropleth", type: "text", name: "Right label", sync: this.rightSync, value: this.rightLabel }));
