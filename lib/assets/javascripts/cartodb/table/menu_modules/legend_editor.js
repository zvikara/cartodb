...
cdb.admin.mod.LegendEditorItem = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    _.bindAll(this, "_onToggleShowTitle");
  ...
  render: function() {
    ...
    var values = _.extend( this.model.toJSON(), { title: this.title, show_switch: this.showSwitch, removable: this.removable });
  ...
  _onToggleShowTitle:function() {
    ...
    if (this.model.get("show_title")) this.$el.find(".title").addClass("enabled").removeClass("disabled");
    else this.$el.find(".title").addClass("disabled").removeClass("enabled");
