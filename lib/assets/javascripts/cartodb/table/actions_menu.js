  ...
  cdb.admin.LayersDialog = cdb.core.View.extend({
    className: 'layers_dialog',

    initialize: function() {
      // New visualization header
      this.newHeader = new cdb.admin.NewHeader({
        globalError: this.options.globalError,
        model: this.options.master_vis,
        visualization: this.options.vis,
        config: cdb.config,
        user: this.options.user,
        geocoder: this.options.geocoder
      });
      this.addView(this.newHeader);
      this.$el.prepend(this.newHeader.render().el);

      // add tabs
      this.$el.append('<div class="map_elements_tabs"> <a href="#layers">layers</a><a href="#element">elements</a></div>');
      this.$el.append('<div class="layerlist"></div>');
      this.tabs = new cdb.admin.Tabs({
          el: this.$('.map_elements_tabs'),
          slash: false
      });
      this.tabs.bind('click', function(name) {
        if (name === 'layers') {
          this.layers.showAll();
        }
      }, this);
      ...
      // add layers
      this.layers = new cdb.admin.LayersPanel({
        vis: this.options.vis,
        master_vis: this.options.master_vis,
        user: this.options.user,
        globalError: this.options.globalError
      });

      this.$el.append(this.layers.render().el);
    }
  });
  ...
  cdb.admin.LayersPanel = cdb.ui.common.TabPane.extend({
    ...
    showAll: function() {
      if (this.activePane) {
        ...
        this.vis.set('active_layer_id', null);
      }
      this.render();
      this._manageLayers();
    },
    ...
    _addLayers: function(layers) {
      ...
      this._manageLayers();
      this.trigger('switch', this.layer_panels[0])
    },
    ...
    _manageLayers: function() {
      ...
      // Add active class
      if (this.activePane) {
        this.$('.layer_panel').not('.layer_panel.active').css({ height: 0 });
      } else {
