  ...
  cdb.admin.LayerPanelView = cdb.admin.RightMenu.extend({
    ...
    setDataLayer: function(dataLayer) {
      ...
      function gotoMap() {
        window.table_router.navigate(this._generateTableUrl(true), {trigger: true});
      };
      
      self.addModule(wizards.render(),      ['map', 'mapLite']).bind('click', gotoMap, this);
      self.addModule(infowindow.render(),   ['map', 'mapLite']).bind('click', gotoMap, this);
      //self.addModule(editorPanel.render(),  ['map', 'mapLite']);
      self.addModule(legends.render(),      ['map', 'mapLite']).bind('click', gotoMap, this);;
      var filters = self.addModule(this.filters.render(), ['table', 'tableLite', 'map', 'mapLite']);
      filters.bind('click', function() {
        window.table_router.navigate(this._generateTableUrl(false), {trigger: true});
      }, this);
    ...
    //TODO i guess this is better done somewhere
    _generateTableUrl: function(maptable) {
      ...
      if (this.vis.isVisualization()) {
        ...
        if (!isOwner) {
          ...
          url += '/tables/' + owner_username + '.' + this.vis.get('table').name;
        } else {
          url += '/tables/' + this.vis.get('table').name;
        }
      }

      // Get scenario parameter from event or current url (table or map)
      url += maptable ? '/map': '/table'
