  ...
  cdb.admin.WMSService = Backbone.Model.extend({
    url: function() {
      return '/wms-service?wms_url=' + encodeURIComponent(this.get('wms_url'));
    },

    // TODO: wms url -> json, remove this when ready
    fetch: function() {
      var self = this;

      if(self.get('wms_url') !== 'http://basemap.nationalmap.gov/arcgis/services/USGSImageryTopo/MapServer/WMSServer?service=WMS&request=GetCapabilities') {
        this.trigger('error');
      } else {
        setTimeout(function() {
          self.set({
  ...
  cdb.admin.WMSBasemapChooserPane = cdb.admin.BasemapChooserPane.extend({
    ...
    initialize: function() {
      _.bindAll(this, "_errorChooser", "_onInputChange", "checkTileJson");
    ...
    checkTileJson: function(val) {
      ...
      w = new cdb.admin.WMSService({ wms_url: val });

      w.bind('change:layers', function() {
        self.trigger('chooseWMSLayers', w);
      });

      w.bind('error', function() {
        self._errorChooser();
      });
