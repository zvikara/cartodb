  ...
  cdb.admin.WMSService = Backbone.Model.extend({
    url: function() {
      ...
      // TODO: append required params to the URL
      //if (url.toLowerCase().indexOf("request=getcapabilities") == -1) url += "&request=GetCapabilities";
      //if (url.toLowerCase().indexOf("service=wms") == -1)             url += "&service=WMS";

      return '/api/v2/wms?url=' + encodeURIComponent(url);
    }
  });
