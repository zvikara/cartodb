  ...
  cdb.admin.BaseMapAdder = cdb.admin.BaseDialog.extend({
    ...
    _updateBoundingBox: function() {
      ...
      if (bboxes && bboxes.length > 0) {
        ...
        if (!bbox) {
          var projection = bboxes[0].srs || bboxes[0].crs;
          bbox =  { bounding_box: bboxes[0], projection: projection.replace(/epsg:/gi, "") };
        }


        var xmin = bbox.bounding_box.minx;
        var ymin = bbox.bounding_box.miny;
        var xmax = bbox.bounding_box.maxx;
        var ymax = bbox.bounding_box.maxy;
        var srid = bbox.projection;

        //if (bbox.projection == 3857) {
          //console.log("miny: " + bbox.bounding_box.miny, "maxx: " + bbox.bounding_box.maxx, "maxy: " + bbox.bounding_box.maxy, "minx: " + bbox.bounding_box.minx);
          //setTimeout(function() {
          //table.map.setBounds([[bbox.bounding_box.miny, bbox.bounding_box.maxx],[bbox.bounding_box.maxy, bbox.bounding_box.minx]]);
          //}, 1000);
        //} else {

          sqlView.setSQL("WITH bbox as (SELECT (ST_Transform(ST_MakeEnvelope(" + xmin + ", " + ymin + ", " + xmax + ", " + ymax + ", (SELECT srid FROM spatial_ref_sys WHERE auth_srid = " + srid + ")), 4326)) as bbox_t) SELECT ST_XMin(bbox_t) as xmin, ST_YMin(bbox_t) as ymin, ST_XMax(bbox_t) as xmax, ST_YMax(bbox_t) as ymax FROM bbox");
          sqlView.bind("reset", function(data) {
            var bbox = data.models[0];
            console.log(bbox.attributes);
            table.map.setBounds([
              [bbox.attributes.ymin, bbox.attributes.xmax],
              [bbox.attributes.ymax, bbox.attributes.xmin]
            ]);
            debugger;
          });
    ...
    _successWMSChooser: function(data) {
      ...
      sqlView = new cdb.admin.SQLViewDataAPI();
