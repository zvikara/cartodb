...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    _.bindAll(this, "_renderOverlay", "_addOverlays", "_bindMapOptions", "_bindOverlays", "_exportImage");
  ...
  _showGMapsDeprecationDialog: function() {
    ...
    var dialog = new cdb.admin.BaseDialog({ 
      $el: $("body"),
      width: 600,
      modal_type: "confirmation warning",
      template_name: "table/views/gmaps_deprecation_dialog",
      title: "We are removing the support for Google Maps",
      description: "Since this visualization uses a Google Maps basemap, if you click 'Ok' we'll automatically change your basemap to an equivalent one provided by Leaflet. Click 'cancel' to go back to your dashboard without changing anything.<br /><br />If you have any questions regarding this change, please contact us at <a href='mailto:support@cartodb.com'>support@cartodb.com</a>",
    ...
    this.$el.parent().append(dialog.render().el);
  ...
  _addBasemapDropdown: function() {
    ...
    if (!this.basemapDropdown) {
      ...
      this.basemapDropdown = new cdb.admin.DropdownBasemap({
        ...
        available_basemaps: this.options.user.get("available_basemaps"),
        tick: "left",
        vertical_offset: 40,
        horizontal_position: "left",
  ...
  _positionLoaderOverlay: function(overlay_model) {
    ...
    else if (this.fullscreen) y = this.fullscreen.model.get("y") + 45;
  ...
  _bindMapOptions: function() {
    ...
    this.mapOptionsDropdown.bind("onExportImage", function() {
      self._exportImage();
    });
  ...
  _exportImage: function() {
    ...
    var dialog = new cdb.admin.StaticImageDialog({
      vis:           this.vis,
      layergroup_id: this.mapView.groupLayer.layerToken,
