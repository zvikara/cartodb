cdb.admin.MapOptionsDropdown = cdb.ui.common.Dropdown.extend({
  ...
  events: {
    "click" : "killEvent",
    "click li.export_image a" : "_onExportImageClick",
  },
  ...
  _setupOptions: function() {
    ...
    if (!this.options.vis.map.get("legends"))     this.mapOptions.set({ legends: false });
  ...
  _onExportImageClick: function(e) {

    this.killEvent(e);
    this.trigger("onExportImage", this);
  ...
  render: function() {
    ...
    // Can this user hide CartoDB logo?
    if (this.options.user.get('actions').remove_logo) {
      switches.push('logo');
    } else {
      this.$('.logo').remove();
    }
