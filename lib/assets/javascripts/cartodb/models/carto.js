...
function cluster_sql(table, zoom, props, nquartiles) {
  ...
  var sql = "WITH meta AS ( " +
    " SELECT ext, ST_XMin(ext) xmin, ST_YMin(ext) ymin, greatest(!pixel_width!,!pixel_height!) as psz " +
    " FROM " +
    " (SELECT !bbox! AS ext) a),"+
    " filtered_table AS ( " +
    "   SELECT t.the_geom_webmercator as g, t.cartodb_id AS id "+
    " FROM <%= table %> t,"+
    " meta m WHERE t.the_geom_webmercator && m.ext), ";

  for (var i = 0; i<nquartiles; i++) {
    bucket = "bucket" + grids[i];

    if (i == 0){
      sql += mainBucket + " AS (SELECT st_collect(g) as g, count(*) AS points_count, 1 as id, array_agg(id) as id_list "
    }

    if (i > 0){
      sql += "\n" + bucket + " AS (SELECT st_collect(g) as g, count(*) as points_count, 1 as id, array_agg(id) as id_list "
    }

    sql += " FROM filtered_table, meta m "

    if (i == 0){
      sql += " GROUP BY ST_SnapToGrid(g, m.psz * <%= size %>, m.psz * <%= size %>) HAVING count(*) > <%= size %>),";
    }

    if (i > 0){
      sql += " WHERE id NOT IN (select unnest(id_list) FROM " + mainBucket + ") ";

      for (var j = 1; j<i; j++) {
        bucket2 = "bucket" + grids[j];
        sql += " AND id NOT IN (select unnest(id_list) FROM " + bucket2 + ") ";
      }

      if (i == nquartiles - 1) {
        sql += "GROUP BY st_snaptogrid(g, m.psz * 0.5 * <%= size %>, m.psz * 0.5 * 48) HAVING count(*) > GREATEST(<%= size %> * 0.1, 2))";
      } else {
        sql += " GROUP BY ST_SnapToGrid(g, m.psz * " + sizes[i] + " * <%= size %>, m.psz * " + sizes[i] + " * <%= size %>) HAVING count(*) > <%= size  %>*0.5), ";
      }
  ...
  sql += " SELECT g AS the_geom_webmercator, 1 points_count, id as cartodb_id, ARRAY[id] as id_list, 'origin' as src, id::text cdb_list FROM filtered_table WHERE ";

  for (var i = 0; i < nquartiles; i++) {
    bucket = "bucket" + grids[i];
    sql += "\n" + (i > 0 ? "AND " : "") + "id NOT IN (select unnest(id_list) FROM " + bucket + ") ";
  }

  for (var i = 0; i < nquartiles; i++) {
    bucket = "bucket" + grids[i];
    sql += " UNION ALL SELECT ST_Centroid(g), points_count, id, id_list, '" + bucket + "' as src, array_to_string(id_list, ',') cdb_list FROM " + bucket
  }
