describe("Dashboard visualizations view", function() {
  ...
  describe("Visualizations list", function() {
    ...
    it("shouldn't render any html tag in the visualization description", function() {
      var vis = generateVisData('table_name1', 'test');
      vis.description = "<a href='#/test' onlick='alert(\'paco\')'>hello hello</a> [other link](http://eeeexample.net/)";
      visualizations.reset([ vis ]);
      var vis_item = _.find(view._subviews, function() { return true });
      expect(vis_item.$('.vis-description').html()).toBe('hello hello other link');

      vis.description = "<a href='#/hum' onlick='alert(\'test\')'>jarrr</a>";
      visualizations.reset([ vis ]);
      var table_item = _.find(view._subviews, function() { return true });
      expect(table_item.$('.vis-description').html()).toBe('jarrr');

      vis.description = "[This link](http://example.net/)";
      visualizations.reset([ vis ]);
      var table_item = _.find(view._subviews, function() { return true });
      expect(table_item.$('.vis-description').html()).toBe('This link');

      vis.description = "";
      visualizations.reset([ vis ]);
      var table_item = _.find(view._subviews, function() { return true });
      expect(table_item.$('.vis-description').length).toBe(0);
    });
