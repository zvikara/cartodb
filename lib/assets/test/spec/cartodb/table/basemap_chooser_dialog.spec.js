describe("Basemap adder dialog", function() {
  ...
  it("should show an error if the wms url template doesn't exist", function() {
    spyOn(basemap_dialog, '_errorChooser');
    basemap_dialog.render();

    var val = basemap_dialog.wmsPane.$el.find("input").val("http://basemap.nationalmap.gov/arcgis/services/USGSImageryTopo/MapServer/WMSServer/error");

    basemap_dialog.wmsPane.checkWMSTemplate(val.val());
    expect(basemap_dialog._errorChooser).toHaveBeenCalled();
  });
